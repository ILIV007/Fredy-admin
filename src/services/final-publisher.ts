...
export class FinalPublisher {
  ...
  async publish(content: ReadyContent): Promise<PublishResult> {
    ...
    // v14.0.8: CRITICAL FIX — wrapped in .catch() to prevent dedup recording
    // from being skipped if history throws. Previously, if history.recordPublished
    // threw (KV quota, network error), the entire publish() function threw,
    // and duplicateDetector.recordPublished() was NEVER called — but the post
    // was already on the channel. This caused duplicates.
    await this.deps.history.recordPublished(content, messageId, chatId).catch(() => {});
