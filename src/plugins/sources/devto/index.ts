/**
 * src/plugins/sources/devto/index.ts — REAL implementation.
 */
...
export class DevToPlugin implements Plugin {
  ...
  normalize(raw: unknown): SourceItem {
    ...
    return {
      id: String(a["id"] ?? ""), source: this.metadata.id, category: this.metadata.category,
      title: String(a["title"] ?? ""), body: String(a["description"] ?? ""),
      url: String(a["url"] ?? ""), language: "en",
      publishedAt: a["published_at"] ? Date.parse(String(a["published_at"])) : undefined,
      imageUrl: a["cover_image"] ? String(a["cover_image"]) : undefined,
      metadata: { tags: a["tag_list"], reactions: a["positive_reactions_count"] }, fetchedAt: Date.now(),
    };
  }

  validate(item: SourceItem): boolean { return !!item.title && !!item.url && item.url.includes("dev.to"); }
