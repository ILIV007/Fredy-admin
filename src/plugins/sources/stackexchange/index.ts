...
export class StackExchangePlugin implements Plugin {
  ...
  async fetch(): Promise<readonly SourceItem[]> {
    ...
    const r = await fetch("https://api.stackexchange.com/2.3/questions?order=desc&sort=hot&site=stackoverflow&pagesize=10");
    if (!r.ok) throw new Error(`SO API ${r.status}`);
