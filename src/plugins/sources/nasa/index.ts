...
export class NasaPlugin implements Plugin {
  ...
  normalize(raw: unknown): SourceItem {
    ...
    return { id: String(apod["date"] ?? ""), source: this.metadata.id, category: this.metadata.category, title: String(apod["title"] ?? ""), body: String(apod["explanation"] ?? ""), url: String(apod["url"] ?? "https://apod.nasa.gov/"), language: "en", publishedAt: apod["date"] ? Date.parse(String(apod["date"])) : undefined, imageUrl, media: { type: mediaType === "video" ? "video" : "image", url: imageUrl, alt: String(apod["title"] ?? ""), source: "provider" }, metadata: { mediaType }, fetchedAt: Date.now() };
  }
