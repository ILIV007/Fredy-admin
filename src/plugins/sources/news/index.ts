/**
 * src/plugins/sources/news/index.ts — REAL implementation.
...
export class NewsPlugin implements Plugin {
  ...
  async fetch(): Promise<readonly SourceItem[]> {
    ...
    const url = `https://newsapi.org/v2/top-headlines?category=technology&language=en&pageSize=10&apiKey=${this.deps.env.NEWSAPI_KEY}`;
  ...
  normalize(raw: unknown): SourceItem {
    ...
    return {
      id: String(a["url"] ?? "").slice(0, 100),
      source: this.metadata.id, category: this.metadata.category,
      title: String(a["title"] ?? ""), body: String(a["description"] ?? a["content"] ?? ""),
      url: String(a["url"] ?? ""), language: "en",
      publishedAt: a["publishedAt"] ? Date.parse(String(a["publishedAt"])) : undefined,
      imageUrl: a["urlToImage"] ? String(a["urlToImage"]) : undefined,
      metadata: { author: a["author"] },
      fetchedAt: Date.now(),
    };
  }
  ...
  async health(): Promise<PluginStatus> {
    return { pluginId: this.metadata.id, healthy: !!this.deps.env.NEWSAPI_KEY, enabled: this.metadata.enabled,
      lastFetchAt: null, lastSuccessAt: null, lastErrorAt: null, lastErrorMessage: this.deps.env.NEWSAPI_KEY ? null : "NEWSAPI_KEY not set",
