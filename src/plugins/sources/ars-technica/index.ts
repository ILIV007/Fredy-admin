/**
 * src/plugins/sources/ars-technica/index.ts
 * v13.0.0: Ars Technica RSS plugin — Tier H.
 *
 * One RSS item = one Strategy candidate. Only the latest article is published
 * per fetch (the strategy engine picks which slot gets it). No daily summaries.
 *
...
import { arsTechnicaManifest } from "./manifest";
export { arsTechnicaManifest } from "./manifest";

const RSS_URL = "https://feeds.arstechnica.com/arstechnica/index";
const CACHE_KEY = "fredy:source:ars-technica:latest";
const CACHE_TTL_SECONDS = 4 * 3600; // 4 hours (Tier H)

export interface ArsTechnicaPluginDeps {
  readonly env: Env;
  readonly kv: KVStore;
...
export class ArsTechnicaPlugin implements Plugin {
  readonly metadata = arsTechnicaManifest;

  constructor(private readonly deps: ArsTechnicaPluginDeps) {}
  ...
  async fetch(): Promise<readonly SourceItem[]> {
    this.deps.logger.info("source.fetch_start", { plugin: "ars-technica" });
    ...
    if (!res.ok) {
      this.deps.logger.warn("source.fetch_error", { plugin: "ars-technica", status: res.status });
      return [];
    }
    ...
    this.deps.logger.info("source.fetch_success", { plugin: "ars-technica", returned: items.length });
    return items;
  }
    ...
    while ((match = itemRegex.exec(xml)) !== null && items.length < 10) {
      ...
      if (title && link) {
        items.push({
          ...
          displaySource: this.metadata.displaySource ?? "Ars Technica",
          fetchedAt: Date.now(),
        });
      }
    }
  ...
  validate(item: SourceItem): boolean {
    return !!item.title && !!item.url && (item.url.includes("arstechnica.com"));
  }

  /** v13.0.0: Quality filter — boost tech/hardware/science/policy topics. */
  async qualityFilter(item: SourceItem): Promise<ProviderQualityResult | null> {
    ...
    const PREFERRED = ["hardware", "cpu", "gpu", "chip", "processor", "tech", "science", "ai", "policy", "security"];
    ...
    return { item, score, reason: `topics=${matched.join(",")}`, boost: matched.includes("hardware") || matched.includes("chip") };
  }
...
export function createArsTechnicaPlugin(deps: ArsTechnicaPluginDeps): ArsTechnicaPlugin {
  return new ArsTechnicaPlugin(deps);
}
