/**
 * src/plugins/sources/joke/index.ts — REAL implementation.
...
export class JokePlugin implements Plugin {
  ...
  async fetch(): Promise<readonly SourceItem[]> {
    ...
    const url = "https://v2.jokeapi.dev/joke/Programming?safe-mode=true&amount=5";
    const response = await fetch(url);
    if (!response.ok) throw new Error(`JokeAPI ${response.status}`);
  ...
  normalize(raw: unknown): SourceItem {
    ...
    return {
      id: String(joke["id"] ?? Date.now()), source: this.metadata.id, category: this.metadata.category,
      title: setup, body: punchline, url: "https://v2.jokeapi.dev/", language: "en",
      metadata: { type, setup, punchline }, fetchedAt: Date.now(),
    };
  }
