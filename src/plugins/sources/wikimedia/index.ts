/**
 * src/plugins/sources/wikimedia/index.ts — REAL implementation.
 */
...
export class WikimediaPlugin implements Plugin {
  ...
  normalize(raw: unknown): SourceItem {
    ...
    const page = pages?.[0];
    return {
      id: String(e["text"] ?? "").slice(0, 100), source: this.metadata.id, category: this.metadata.category,
      title: String(e["text"] ?? "Today in History"), body: String(page?.["extract"] ?? e["text"] ?? ""),
      url: String(page?.["content_urls"]?.["desktop"]?.["page"] ?? "https://en.wikipedia.org"),
      language: "en", imageUrl: page?.["thumbnail"]?.["source"] ? String(page["thumbnail"]["source"]) : undefined,
      metadata: { year: e["year"] }, fetchedAt: Date.now(),
    };
  }
