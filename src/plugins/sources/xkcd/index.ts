/**
 * src/plugins/sources/xkcd/index.ts — REAL implementation.
 */
...
export class XkcdPlugin implements Plugin {
  ...
  async fetch(): Promise<readonly SourceItem[]> {
    ...
    const response = await fetch("https://xkcd.com/info.json");
    if (!response.ok) throw new Error(`XKCD API ${response.status}`);
  ...
  normalize(raw: unknown): SourceItem {
    ...
    return {
      id: `xkcd-${num ?? ""}`, source: this.metadata.id, category: this.metadata.category,
      title: String(c["title"] ?? `XKCD #${num ?? ""}`), body: String(c["alt"] ?? ""),
      url: `https://xkcd.com/${num ?? ""}/`, language: "en",
      imageUrl: img, media: { type: "image", url: img, alt: String(c["alt"] ?? ""), source: "provider" },
      metadata: { num, alt: c["alt"] }, fetchedAt: Date.now(),
    };
  }
