...
export class HackerNewsPlugin implements Plugin {
  ...
  async fetch(): Promise<readonly SourceItem[]> {
    ...
    const items = await Promise.all(top.map(async (id) => { const r2 = await fetch(`https://hacker-news.firebaseio.com/v0/item/${id}.json`); return r2.ok ? r2.json() : null; }));
    return items.filter(Boolean).map((i) => this.normalize(i));
  }

  normalize(raw: unknown): SourceItem {
    const item = raw as Record<string, unknown>;
    return { id: String(item["id"] ?? ""), source: this.metadata.id, category: this.metadata.category, title: String(item["title"] ?? ""), body: String(item["text"] ?? item["url"] ?? ""), url: String(item["url"] ?? `https://news.ycombinator.com/item?id=${item["id"] ?? ""}`), language: "en", publishedAt: item["time"] ? Number(item["time"]) * 1000 : undefined, metadata: { score: item["score"], by: item["by"] }, fetchedAt: Date.now() };
  }
