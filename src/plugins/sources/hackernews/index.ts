 ...
 * Category B (tech news / developer ecosystem news).
 */
...
export class HackerNewsPlugin implements Plugin {
  ...
  async fetch(): Promise<readonly SourceItem[]> {
    ...
    // TODO: implement real fetch in production polish.
    // 1. GET /topstories.json (returns array of IDs)
    // 2. GET /item/<id>.json for each (parallel, take first 10)
    // 3. Filter: score > 50, has URL (not Ask HN)
    return [];
  }
  ...
  normalize(raw: unknown): SourceItem {
    ...
    return {
      ...
      body: String(item["text"] ?? item["url"] ?? ""),
      url: String(item["url"] ?? `https://news.ycombinator.com/item?id=${item["id"] ?? ""}`),
      language: "en",
      publishedAt: item["time"] ? Number(item["time"]) * 1000 : undefined,
      metadata: { score: item["score"], by: item["by"] },
      fetchedAt: Date.now(),
    };
  }

  validate(item: SourceItem): boolean {
    return !!item.title && !!item.url && item.title.length > 5;
  }
